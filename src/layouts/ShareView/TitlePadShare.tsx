...
export default function TitlePadShare({title, createAt}: {title: string, createAt: Timestamp}) {
  return (
    <div className={`relative h-[100px] w-full overflow-hidden flex justify-center items-center`}>
      <img
      ...
      <span className="absolute right-3 bottom-4">Ngày tạo: {toDateTime(createAt)}</span>
    </div>
  );
}
